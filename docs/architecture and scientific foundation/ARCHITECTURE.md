...
**Snapshot reviewed:** source bundles generated 2026-09-02 23:33 CEST  
...
- one unified node-provider contract independent of storage/computation strategy;
...
- spherical Voronoi construction through the Euclidean origin-cell reduction, including antipodal quotient mode.
...
highvoronoi/core.hpp
        ...
        +-- highvoronoi/mesh_engines.hpp
                optional engine-backed/import mesh infrastructure

highvoronoi/highvoronoi.hpp
        complete convenience umbrella
```

`voronoi.hpp` is the narrow entry point for ordinary Euclidean construction. `high_voronoi.hpp` is the narrow entry point for persistent incremental/periodic HighVoronoi. `mesh_engines.hpp` keeps optional engine-backed/import mesh infrastructure out of translation units that do not need it. Engine-backed coordinates and explicitly stored coordinates do **not** define different node-access modes; they are implementation details behind the common node-provider contract.
...
All generic node providers return the same owning `PointType<Scalar, Dim>` from convenience accessors. Concrete providers may additionally expose storage-specific optimizations, but generic mesh/search/geometry code does not depend on them.

### 4.2 Unified node-provider contract

`AbstractVoronoiNodes<Scalar, Dim, Index>` is the common node interface. The former compile-time `Stored` / `Computed` / `Hybrid` access-mode hierarchy is no longer part of the architecture.

Every provider exposes the same semantic operations:

- `size()` for the addressable node count;
- `get_data(node, coordinate)` for coordinate-wise hot paths;
- `copy_node(node, target)` for caller-owned reusable point storage;
- owning `operator[]` / `node()` convenience access through one common `PointType`.

Whether a provider stores coordinates, computes them, or routes among several sources is private provider state. `VoronoiNodes` is the concrete contiguous provider; `CompositeVoronoiNodes` provides one stable global index space over engine ranges plus explicitly materialized nodes; `MappedVoronoiNodes` is a non-owning facade applying a public/internal mapping. `VoronoiNodes::stable_node_data()` remains a concrete optimization only and is not part of the generic contract.
...
- integration of a compile-time public/internal mapping policy.

Concrete meshes provide node, boundary, database and address-list storage hooks. Public/internal ordinary-node mapping is supplied by the mapping policy rather than by the removed node-access-mode machinery.

### 6.2 Mapping policies and mapped node facades

`DenseIndexMapping` is the ordinary bijective mapping used when every public node has one current stable-internal representative. Internal slots remain stable while public numbering may be rebuilt densely after deletion.

`ProjectedIndexMapping` is the deliberately many-to-one HighVoronoi projection: `public_to_internal` selects one canonical visible representative, while active invisible periodic copies may map back to the same visible public index.

`MappedVoronoiNodes` applies either mapping to an underlying node provider without owning the provider or the mapping. Compute/view node access therefore stays independent of concrete mesh field layout.

### 6.3 `VoronoiMesh`

`VoronoiMesh` is the ordinary persistent concrete mesh.

It owns:

- stable ordinary node storage;
- a dense public/internal mapping;
- extended boundary nodes;
- the persistent vertex database;
- primary and secondary vertex-address lists;
- mesh-level infinite-edge addresses.

Public node deletion changes the dense public mapping without rewriting stable internal signatures.

### 6.4 `ReorderedMeshView`
...
### 6.5 Optional engine-backed source meshes

`ComputeMeshEngine` defines the optional analytic/import-engine contract. `ComputeMesh<Engine>` exposes one such engine through the ordinary `AbstractMesh` read/erase/contains interface without materializing all engine vertices. `CuboidMeshEngine` is the concrete Cartesian analytic engine in the supplied source.

`HybridDataBase` is a separate optional logical-address router that combines ordinary stored database records with engine-reserved address ranges while sharing one persistent signature hash. Its name does **not** reintroduce a `Hybrid` node-access mode. None of these engine components changes the common `AbstractVoronoiNodes` contract.

The engine subsystem is not the foundation of the ordinary incremental or HighVoronoi geometry kernel; it is an optional source/import path.

### 6.6 `HighVoronoiMesh`

`HighVoronoiMesh` is the central persistent owner for incremental and periodic HighVoronoi state. Its internal nodes are exposed through `CompositeVoronoiNodes`, so explicitly materialized nodes and optional engine ranges share one stable ordinary internal index space without creating different node interface types.
...
### 6.7 Invisible periodic reference nodes
...
### 6.8 Public HighVoronoi projection
...
### 6.12 `SphericalVoronoiMesh`

`SphericalVoronoiMesh` is implemented as a reduction to one ordinary Euclidean origin cell in `R^d`. Internal node 0 is the origin; user generators lie on the unit sphere; only the origin cell is computed. Stored Euclidean vertices are projected radially onto the sphere and the origin is removed from public signatures.

Two public topologies are supported:

- `Sphere`;
- `AntipodalHemisphere`, which appends invisible antipodes and identifies `q` with `-q`, giving the quotient `S^(d-1)/{x ~ -x}` (the unit-quaternion representation of `SO(3)` when `d=4`).

The class exposes initial `compute()` and incremental `refine()` / `remove()` workflows while reusing the ordinary `ComputeVoronoi` kernel.
...
- point layout;
- the unified node-provider contract and concrete provider behavior;
- boundaries;
...
- infinite-edge storage;
- edge closure;
- optional engine-backed mesh/database routing;
...
- configuration documentation outside this Architecture/README still needs to catch up with the HighVoronoi and vertex-correction layers;
- `detail/neighbour_database.hpp` is present as a standalone low-level record store, but no other header in the supplied current source snapshot includes or references it. It must not be treated as an active mesh-neighbour subsystem without a later integration step.
...
6. continue profiling the validated incremental/periodic and spherical paths;
7. either integrate or remove standalone neighbour-storage remnants so file presence cannot be mistaken for active architecture.

### Longer-term roadmap

1. fast quasi-periodic generation;
2. optimized fused `direct_cast` backends;
3. additional nearest-neighbour backends where profiling justifies them;
4. optional disk-backed storage;
5. volume/interface/quadrature/integral algorithms.
...
- [ ] Generic algorithms depend only on the unified node-provider contract and never require storage-specific stable pointers.
...
- [ ] Existing persistent vertices enter only cell-local queue/edge state.
...
| Precision helpers | `include/highvoronoi/algorithm/detail/float.hpp` |
| Locks / execution policies | `include/highvoronoi/core/detail/locks.hpp` |
| Atomic structural/event flags | `include/highvoronoi/core/detail/atomic_bit_vector.hpp` |
| Hash functions/generators | `include/highvoronoi/storage/hash/hash_functions.hpp`, `hash_generators.hpp` |
...
| Persistent database | `include/highvoronoi/storage/hvdatabase.hpp` |
| Optional engine/stored logical-address router | `include/highvoronoi/storage/hybrid_database.hpp` |
| Address lists | `include/highvoronoi/storage/read_write_list.hpp` |
| Index views | `include/highvoronoi/mesh/detail/hvview.hpp` |
| Points | `include/highvoronoi/core/point.hpp` |
| Unified node providers / composite node space | `include/highvoronoi/core/voronoi_nodes.hpp` |
| Public/internal mapping policies | `include/highvoronoi/mesh/mesh_index_mapping.hpp` |
| Non-owning mapped-node facade | `include/highvoronoi/mesh/mapped_voronoi_nodes.hpp` |
| Boundaries / periodic pairs | `include/highvoronoi/core/boundary.hpp` |
| Mesh base | `include/highvoronoi/mesh/abstract_mesh.hpp` |
| Ordinary persistent mesh | `include/highvoronoi/mesh/voronoi_mesh.hpp` |
| Mesh views | `include/highvoronoi/mesh/mesh_view.hpp` |
| Engine-backed read-oriented mesh facade | `include/highvoronoi/mesh/engine/compute_mesh.hpp` |
| Compute engine base | `include/highvoronoi/mesh/engine/compute_mesh_engine.hpp` |
| Cuboid compute engine | `include/highvoronoi/mesh/engine/cuboid_mesh_engine.hpp` |
| Spherical / antipodal spherical mesh | `include/highvoronoi/mesh/spherical_voronoi_mesh.hpp` |
| Standalone neighbour-record store (not integrated in supplied snapshot) | `include/highvoronoi/storage/neighbour/neighbour_database.hpp` |
| HighVoronoi owner | `include/highvoronoi/mesh/high_voronoi_mesh.hpp` |
| HighVoronoi compute facade/event DB | `include/highvoronoi/mesh/high_voronoi_compute_mesh.hpp` |
| Visible-first HighVoronoi view | `include/highvoronoi/mesh/visible_first_mesh.hpp` |
| Generic incremental compute facade | `include/highvoronoi/algorithm/incremental/incremental_voronoi_compute_mesh.hpp` |
| Incremental backend dispatch | `include/highvoronoi/algorithm/incremental/incremental_voronoi_backend.hpp` |
| HighVoronoi incremental backend | `include/highvoronoi/algorithm/high_voronoi/high_voronoi_incremental_backend.hpp` |
| Incremental insertion | `include/highvoronoi/algorithm/incremental/refine_voronoi.hpp` |
| Incremental removal | `include/highvoronoi/algorithm/incremental/remove_voronoi.hpp` |
| HighVoronoi orchestration | `include/highvoronoi/algorithm/high_voronoi/compute_high_voronoi.hpp` |
| Hidden periodic-boundary repair | `include/highvoronoi/algorithm/high_voronoi/periodic_boundary_repair.hpp` |
| Search base | `include/highvoronoi/search/abstract_search_tree_crtp.hpp` |
| Brute-force search | `include/highvoronoi/search/brute_force_search_tree_crtp.hpp` |
| nanoflann search | `include/highvoronoi/search/nanoflann_search_tree_crtp.hpp` |
| Search factory | `include/highvoronoi/search/search_tree_factory_crtp.hpp` |
| Bundled nanoflann | `include/highvoronoi/search/detail/nanoflann.hpp` |
| Normal/vertex solving | `include/highvoronoi/algorithm/normal_solver.hpp` |
| Edge enumeration | `include/highvoronoi/algorithm/edge_iterator.hpp` |
| Ray casting / verification | `include/highvoronoi/algorithm/raycaster.hpp` |
| Geometry worker | `include/highvoronoi/algorithm/voronoi_worker.hpp` |
| Cell/branch construction | `include/highvoronoi/algorithm/systematic_voronoi.hpp` |
| Ordinary top-level construction | `include/highvoronoi/algorithm/compute_voronoi.hpp` |
| Mesh validation | `include/highvoronoi/mesh/validation/mesh_validation.hpp` |
| Selected-cell validation | `include/highvoronoi/mesh/validation/mesh_cell_validation.hpp` |
| Ordinary incremental regression | `tests/workflows/incremental/test_voronoi_incremental_workflows.cpp` |
| HighVoronoi workflow regression | `tests/workflows/high_voronoi/test_high_voronoi_nonperiodic_workflow.cpp` |
| HighVoronoi incremental regression | `tests/workflows/high_voronoi/test_high_voronoi_incremental_vs_batched.cpp` |
| HighVoronoi compute mapping regression | `tests/workflows/high_voronoi/test_high_voronoi_compute_view_range_equivalence.cpp` |
| Full periodic reference regression | `tests/workflows/high_voronoi/test_high_voronoi_periodic_reference.cpp` |
| Periodic incremental reference regression | `tests/workflows/high_voronoi/test_high_voronoi_periodic_incremental_reference.cpp` |
| Periodic neighbour/reference regression | `tests/workflows/high_voronoi/test_voronoi_periodic_refine_edge_neighbours.cpp` |
| Vertex solver regression | `tests/algorithm/numerics/test_qr_normal_solver_20260807.cpp` |
| Parallel smoke regression | `tests/algorithm/compute/test_compute_voronoi_parallel_smoke.cpp` |
| Unbounded completeness regression | `tests/algorithm/compute/test_compute_voronoi_unbounded_completeness_20260814.cpp` |
