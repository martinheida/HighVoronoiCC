...
// ============================================================================
// Test 3: cubical grid + Julia-faithful CombinedRayCaster
// ============================================================================

void test_combined_raycast_on_degenerate_cube() {
    std::cout << "\n============================================================\n";
    std::cout << "[TEST] cubical grid: EdgeIterator + CombinedRayCaster\n";
    std::cout << "============================================================\n";

    Mesh mesh = make_cubical_grid_mesh();
    const Sigma origin_sigma = first_indices(central_cube_node_count);
    const Point origin = zero_point();
    (void)mesh.store_vertex(origin, origin_sigma);
    const auto stored = load_vertex(mesh, Index{0});

    auto tree = highvoronoi::geometry::make_search_tree(
        mesh, highvoronoi::geometry::KDSearch{8, 1});
    ...
    check(
        raycaster.parameters().variance_tolerance == parameters.variance_tolerance,
        "CombinedRayCaster is constructed through RaycastParameters");

    Iterator iterator(raycaster.extended_nodes());
    const std::vector<Index> no_boundary_neighbours;
    const std::vector<Point> expected = expected_direct_neighbour_vertices();
    std::vector<bool> seen(expected.size(), false);
    std::size_t raycast_count = 0;

    for (Index cell = Index{0}; cell < central_cube_node_count; ++cell) {
        raycaster.activate_cell(cell, no_boundary_neighbours);
        iterator.reset(stored.sigma, stored.position, cell, OnQueueEdges{});

        while (const auto edge_view = iterator.next()) {
            const Candidate edge(*edge_view);
            Sigma endpoint_sigma(edge.full_indices.begin(), edge.full_indices.end());
            const auto result = raycaster.cast(
                endpoint_sigma,
                stored.position,
                edge.direction,
            ...
            check(
                result.status == highvoronoi::RayCastStatus::Finite,
                "combined raycast finds a finite adjacent cube vertex");
            if (result.status != highvoronoi::RayCastStatus::Finite) continue;

            check(
                endpoint_sigma.size() == static_cast<std::size_t>(central_cube_node_count),
                "combined endpoint contains exactly 2^d generators");
            check(
                raycaster.verify_vertex(endpoint_sigma, result.position),
                "combined endpoint is a valid degenerate Voronoi vertex");

            const auto match = matching_expected_vertex(result.position, expected);
            check(match.has_value(),
                  "combined endpoint is one of the expected +/-2 e_i neighbours");
            if (match) {
                check(!seen[*match],
                      "combined direct neighbour is reached only once");
                seen[*match] = true;
            }
        }
    }
    ...
    check(
        raycast_count == static_cast<std::size_t>(2 * Dimension),
        "CombinedRayCaster traverses exactly 2d cube edges");
    check(
        std::all_of(seen.begin(), seen.end(), [](bool value) { return value; }),
        "CombinedRayCaster finds all 2d direct neighbouring cube centers");
}


// ============================================================================
// Test 4: general position + CombinedRayCaster
// ============================================================================

void test_combined_raycast_on_general_vertex() {
    std::cout << "\n============================================================\n";
    std::cout << "[TEST] general position: EdgeIterator + CombinedRayCaster\n";
    std::cout << "============================================================\n";
    ...
    for (Index cell = Index{0}; cell < general_base_node_count; ++cell) {
        raycaster.activate_cell(cell, no_boundary_neighbours);
        iterator.reset(stored.sigma, stored.position, cell, OnQueueEdges{});
        while (const auto edge_view = iterator.next()) {
            const Candidate edge(*edge_view);
            Sigma endpoint_sigma(edge.full_indices.begin(), edge.full_indices.end());
            const auto result = raycaster.cast(
                endpoint_sigma, stored.position, edge.direction,
                edge.indices, stored.sigma,
                highvoronoi::RayCastUsage::WalkRay, edge.cycle_error);
            ++raycast_count;

            check(result.status == highvoronoi::RayCastStatus::Finite,
                  "combined general raycast finds a finite adjacent vertex");
            if (result.status != highvoronoi::RayCastStatus::Finite) continue;
            check(endpoint_sigma.size() == static_cast<std::size_t>(Dimension + 1),
                  "combined general endpoint has exactly d+1 generators");
            check(raycaster.verify_vertex(endpoint_sigma, result.position),
                  "combined general endpoint is a valid Voronoi vertex");
        }
    }

    check(raycast_count == static_cast<std::size_t>(Dimension + 1),
          "CombinedRayCaster traverses all d+1 general-position edges");
}

// ============================================================================
// Test 5: Combined `search_vertex2` boundary-mirror prepass
// ============================================================================

void test_combined_boundary_mirror_prepass() {
    std::cout << "\n============================================================\n";
    std::cout << "[TEST] CombinedRayCaster boundary mirror prepass\n";
    std::cout << "============================================================\n";

    constexpr int D2 = 2;
    using Database2 = highvoronoi::HVDataBase<
        highvoronoi::EmptyLock,
        DatabaseParameters,
        D2>;
    using Mesh2 = highvoronoi::VoronoiMesh<Scalar, D2, Database2>;
    using Point2 = Mesh2::NodePoint;
    using Nodes2 = Mesh2::InternalNodes;
    using Boundary2 = Mesh2::BoundaryType;

    auto point2 = [](Scalar x, Scalar y) {
        Point2 p;
        p << x, y;
        return p;
    };

    Nodes2 nodes(Index{2});
    nodes.set(Index{0}, point2(0.25, 0.5));
    nodes.set(Index{1}, point2(0.75, 0.5));

    Boundary2 boundary;
    boundary.add(highvoronoi::Plane<D2, Scalar, Index>(
        point2(0.0, 0.0), point2(0.0, -1.0),
        highvoronoi::BoundaryCondition::Dirichlet));
    boundary.add(highvoronoi::Plane<D2, Scalar, Index>(
        point2(0.0, 1.0), point2(0.0, 1.0),
        highvoronoi::BoundaryCondition::Dirichlet));

    auto database = std::make_shared<Database2>(
        256, DatabaseParameters{highvoronoi::DirectHash{256}});
    Mesh2 mesh(std::move(nodes), std::move(boundary), std::move(database));

    auto tree = highvoronoi::geometry::make_search_tree(
        mesh, highvoronoi::geometry::KDSearch{1, 1});
    highvoronoi::RaycastParameters<highvoronoi::CombinedRaycast, Scalar> parameters;
    auto raycaster = highvoronoi::make_raycaster(tree, parameters);

    const Index bottom_mirror = Index{2};
    const Index top_mirror = Index{3};
    const std::vector<Index> active_boundary{bottom_mirror, top_mirror};
    raycaster.activate_cell(Index{0}, active_boundary);

    std::vector<Index> sigma{Index{0}, Index{1}};
    const std::vector<Index> edge{Index{0}, Index{1}};
    const std::vector<Index> origin{Index{0}, Index{1}, bottom_mirror};
    const Point2 start = point2(0.5, 0.0);
    const Point2 direction = point2(0.0, 1.0);

    const auto result = raycaster.cast(
        sigma, start, direction, edge, origin,
        highvoronoi::RayCastUsage::WalkRay, Scalar{0});

    check(result.status == highvoronoi::RayCastStatus::Finite,
          "combined boundary prepass finds a finite endpoint");
    check(result.generator == top_mirror,
          "combined boundary prepass selects the upper mirror generator");
    check((result.position - point2(0.5, 1.0)).norm() <= point_tolerance,
          "combined boundary prepass reaches the upper strip boundary");
    check(std::binary_search(sigma.begin(), sigma.end(), top_mirror),
          "combined boundary generator is merged into the endpoint sigma");
}
...
int main() {
    ...
    test_combined_raycast_on_degenerate_cube();
    test_combined_raycast_on_general_vertex();
    test_combined_boundary_mirror_prepass();
