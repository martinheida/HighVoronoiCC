...
void test_degenerate_boundary_last_owner() {
    ...
    using Database3 = highvoronoi::HVDataBase<
        ...
        DatabaseParameters,
        3>;
    using Mesh3 = highvoronoi::VoronoiMesh<Scalar, 3, Database3>;
    ...
    const auto database3 = std::make_shared<Database3>(
        256,
        DatabaseParameters{highvoronoi::DirectHash{256}});
    Mesh3 mesh(std::move(nodes), boundary, database3);
