...
using HighMesh = highvoronoi::HighVoronoiMesh<
    ...
    Dimension,
    Database>;
using ComputeRoundState = highvoronoi::HighVoronoiComputeRoundState<HighMesh>;
using ComputeMesh = highvoronoi::HighVoronoiComputeMesh<HighMesh>;
...
bool visible_cell_has_invisible_generator(
    ...
    const auto compute_cell = compute_mesh.compute_public_of_global_internal(internal);
    if (!compute_cell) {
    ...
    for (const auto& vertex : compute_mesh.vertices(*compute_cell)) {
        for (const Index generator : vertex.sigma) {
            ...
            const Index generator_internal =
                compute_mesh.global_internal_node(generator);
...
NormalizedSignature normalize_high_signature(
    ...
    for (const Index generator : sigma) {
        if (generator < compute_mesh.size()) {
            const Index internal = compute_mesh.global_internal_node(generator);
...
bool high_internal_cell_contains_address(
    ...
    const auto public_cell =
        compute_mesh.compute_public_of_global_internal(internal);
