...
using StoredDatabase = highvoronoi::HVDataBase<highvoronoi::ReadWriteLock, DatabaseParameters, Dimension>;
using HybridDatabase = highvoronoi::HybridDataBase<highvoronoi::ReadWriteLock, DatabaseParameters, Dimension>;
...
void check_data_view_is_stable_order(HighMesh& mesh, std::string_view name) {
    ...
    for (Index public_node = Index{0}; public_node < view.size(); ++public_node) {
        const Index internal = view.stable_internal_node(public_node);
        if (!mesh.is_active_internal(internal)) {
            correct = false;
        }
