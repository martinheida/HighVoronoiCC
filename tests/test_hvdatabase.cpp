...
void test_push_and_read_facet(Database& database)
{
    ...
    highvoronoi::StaticPoint<Scalar, 2> r;
    r[0] = Scalar{4};
    r[1] = Scalar{5};

    highvoronoi::DynamicPoint<Scalar> u(3);
    u[0] = Scalar{-1};
    u[1] = Scalar{-2};
    u[2] = Scalar{-3};
    ...
    highvoronoi::StaticPoint<Scalar, 2> read_r;
    read_r[0] = Scalar{0};
    read_r[1] = Scalar{0};

    highvoronoi::DynamicPoint<Scalar> read_u(3);
    read_u[0] = Scalar{0};
    read_u[1] = Scalar{0};
    read_u[2] = Scalar{0};
...
void test_block_crossing(Database& database)
{
    ...
    std::vector<Index> read_sigma;
    std::vector<Scalar> read_r(r.size());
...
void test_facet_block_crossing(Database& database)
{
    ...
    std::vector<Index> read_sigma;
    std::vector<Scalar> read_r(r.size());
    std::vector<Scalar> read_u(u.size());
