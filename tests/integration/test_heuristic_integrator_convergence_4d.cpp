...
highvoronoi::IntegralDataOptions target_options() {
    highvoronoi::IntegralDataOptions o;
...
void cartesian_convergence() {
    std::cout << "\n=== Heuristic / FastPolygon-source 4D Cartesian convergence ===\n";
    ...
    for (std::size_t n = 2; n <= 6; ++n) {
        ...
        compute_full(*mesh);
        Integral source(*mesh, 1, geometry_options(), 16384, 65536);
        auto source_algorithm = highvoronoi::make_fast_polygon_algorithm(source);
        Integral target_integral(*mesh, 4, target_options(), 16384, 65536);
        auto algorithm = highvoronoi::make_heuristic_algorithm(
            target_integral, source, source_algorithm, function,
            highvoronoi::HeuristicOptions{true});
        const Result result = integrate_and_measure(*mesh, target_integral, algorithm);
        print_result("cartesian", n, pts.size(), result);
        check(result.error[0] < 1e-7,
              "Heuristic Cartesian constant component integrates to source unit hypercube volume");
...
void random_refine_convergence() {
    std::cout << "\n=== Heuristic / FastPolygon-source 4D nested random/refine convergence ===\n";
    ...
    Result first = integrate_and_measure(*mesh, target_integral, algorithm);
    print_result("random-refine", 2, target[0], first);
    check(first.error[0] < 1e-7,
          "Heuristic random constant component integrates to source unit hypercube volume initially");
    ...
    for (std::size_t level = 1; level < target.size(); ++level) {
        ...
        check(result.error[0] < 1e-7,
              "Heuristic random constant component integrates to source unit hypercube volume after refine");
