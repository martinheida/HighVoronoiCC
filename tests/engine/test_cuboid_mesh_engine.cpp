...
void test_geometry_and_incidence() {
    ...
    check(engine.provides_neighbours(),
          "CuboidMeshEngine exposes immutable analytic neighbour records");

    Sigma neighbours;
    check(engine.read_neighbours(Index{0}, neighbours) &&
              neighbours == Sigma({Index{1}, Index{3}, Index{6}}),
          "corner node has exactly its three positive axis neighbours");
    check(engine.read_neighbours(Index{10}, neighbours) &&
              neighbours == Sigma({Index{4}, Index{7}, Index{9}, Index{11}, Index{16}}),
          "node 10 has exactly the valid plus/minus axis neighbours");
    ...
    check(engine.read_neighbours(Index{0}, neighbours) &&
              neighbours == Sigma({Index{1}, Index{3}, Index{6}}),
          "virtual native neighbour record remains immutable after vertex invalidation");
}
