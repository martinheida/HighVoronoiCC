...
#include <highvoronoi/search/brute_force_search_tree_crtp.hpp>
#include <highvoronoi/search/composite_search_tree_crtp.hpp>
...
/**
 * @brief Select nanoflann with a contiguous search-local copy of public nodes.
 *
 * This removes mesh mapping/provider indirection from nanoflann's innermost
 * coordinate-access loop. The snapshot is refreshed whenever the tree is
 * rebuilt.
 */
struct CopyKDSearch {
    std::size_t leaf_max_size = 10;
...
/**
 * @brief Split computed structured nodes from a sparse stored KD-tree overlay.
 */
struct CompositeSearch {
    std::size_t sparse_leaf_max_size = 10;
    unsigned int sparse_build_thread_count = 1;
};
...
template <class Mesh>
struct SearchTreeFactory<CopyKDSearch, Mesh, void> {
    using type = NanoflannSearchTree<Mesh, std::size_t, true>;

    [[nodiscard]] static type make(
        Mesh& mesh,
        const CopyKDSearch& keyword) {
        return type(
            mesh,
            keyword.leaf_max_size,
            keyword.build_thread_count);
...
struct SearchTreeFactory<
    ...
    Mesh,
    std::void_t<decltype(
        detail::SearchTreeNodesTraits<Mesh>::get(
            std::declval<const Mesh&>()).is_stored(
                std::declval<typename Mesh::Index>()))>> {
    using type = CompositeSearchTree<Mesh>;

    [[nodiscard]] static type make(
        Mesh& mesh,
        const CompositeSearch& keyword) {
        return type(
            mesh,
            keyword.sparse_leaf_max_size,
            keyword.sparse_build_thread_count);
    }
};
