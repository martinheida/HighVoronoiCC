...
public:
    ...
    [[nodiscard]] Word load_word(std::size_t word) const noexcept {
        return word < words_.size() ? words_[word] : Word{0};
    }

    void or_word(std::size_t word, Word value) noexcept {
        if (word < words_.size()) {
            words_[word] |= value;
        }
    }

    template<class OtherBitVector>
    void or_assign(const OtherBitVector& other) {
        if (other.size() > size()) {
            resize(other.size(), false);
        }
        const std::size_t count = other.word_count();
        for (std::size_t word = 0; word < count; ++word) {
            or_word(word, other.load_word(word));
        }
        clear_unused_tail_bits();
    }
...
public:
    ...
    [[nodiscard]] Word load_word(std::size_t word) const noexcept {
        return word < word_count_
            ? words_[word].load(std::memory_order_relaxed)
            : Word{0};
    }

    void or_word(std::size_t word, Word value) noexcept {
        if (word < word_count_) {
            words_[word].fetch_or(value, std::memory_order_relaxed);
        }
    }
