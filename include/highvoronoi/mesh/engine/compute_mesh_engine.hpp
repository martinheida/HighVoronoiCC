...
public:
    ...
    /**
     * @brief Return one coordinate of one local node without materializing a Point.
     *
     * Search backends use this coordinate-wise hotpath. Concrete engines must
     * implement it directly so runtime-dimensional node access cannot allocate
     * a temporary point for every coordinate read.
     */
    [[nodiscard]] virtual NodeScalar get_data(
        Index local_node,
    ...
    /**
     * @brief Test an active finite-vertex signature without temporary buffers.
     *
     * The signature is canonical and uses engine-local ordinary-node indices.
     */
    [[nodiscard]] virtual bool contains_vertex_signature(
        const Sigma& sigma) const = 0;
    ...
    // ------------------------------------------------------------------
    // Optional native neighbour support / host-child reconciliation
    // ------------------------------------------------------------------

    /**
     * @brief Whether the engine exposes persistent native facet-neighbour records.
     *
     * Returning true opts the engine into HybridNeighbourDatabase registration:
     * one immutable virtual neighbour record is reserved for every local ordinary
     * cell. read_neighbours(cell, ...) must therefore keep representing that same
     * engine-local baseline topology for the lifetime of the registered engine.
     * Engines whose neighbour state cannot satisfy this persistence contract must
     * return false and let the host materialize/store neighbour records instead.
     *
     * Lists use engine-local ordinary-node indices plus the standard high-end
     * boundary encodings. Multiplicity and self-neighbours are significant.
     */
    [[nodiscard]] virtual bool provides_neighbours() const noexcept {
        return false;
    }

    /**
     * @brief Read the engine's native neighbour list into caller-owned storage.
     *
     * The default reports no list. Concrete engines that opt in should return
     * a sorted list but must not deduplicate it.
     */
    [[nodiscard]] virtual bool read_neighbours(
        Index,
        Sigma& neighbours) const {
        neighbours.clear();
        return false;
    }

    /**
     * @brief Optional host-to-child reconciliation hook.
     *
     * A future composite host may pass a mutable host-derived list translated
     * into this engine's local numbering. A child that chooses to manage some
     * of those facet entries may remove exactly those occurrences from the
     * buffer (and retain whatever mask/state it needs internally). The default
     * consumes nothing. Duplicates are facet multiplicities and must be handled
     * occurrence-by-occurrence.
     */
    virtual void consume_host_neighbours(
        Index,
        Sigma&) {}
