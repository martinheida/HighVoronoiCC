...
private:
    ...
    private:
        void copy_node_impl(
            Index public_index,
            NodeScalar* target) const override {
...
private:
    ...
    [[nodiscard]] bool load_internal_neighbours_impl(
        ...
        return mesh_->internal_neighbours(internal_cell, buffer);
    }

    void store_internal_neighbours_impl(
        Index internal_cell,
        const std::vector<Index>& neighbours) override {
        mesh_->store_internal_neighbours(internal_cell, neighbours);
    }

    [[nodiscard]] bool internal_neighbours_dirty_impl(
        Index internal_cell) const override {
        return mesh_->internal_neighbours_dirty(internal_cell);
    }

    void set_internal_neighbours_dirty_impl(
        Index internal_cell,
        bool value) override {
        mesh_->set_internal_neighbours_dirty(internal_cell, value);
    }

    void compute_neighbors_impl(Index internal_cell) override {
        mesh_->compute_internal_neighbours(internal_cell);
    }
