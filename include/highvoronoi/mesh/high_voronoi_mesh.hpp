...
public:
    ...
    using NeighbourDatabase = detail::HybridNeighbourDatabase<
        typename Database::LockType,
        Index>;
    ...
    using InternalNodes = CompositeVoronoiNodes<NodeScalar, Dim, Index>;
...
private:
    ...
    using PublicVisibleNodes = detail::MappedVoronoiNodes<
        InternalNodes,
        IndexMapping>;
...
public:
    ...
    HighVoronoiMesh(
        ...
        : Base(runtime_dimension, IndexMapping{}),
          internal_nodes_(runtime_dimension),
          visible_nodes_(internal_nodes_, this->index_mapping(), runtime_dimension),
          external_nodes_(std::in_place, visible_nodes_, external_boundary),
          internal_boundary_(std::move(external_boundary)),
          neighbour_workspace_(internal_nodes_, internal_boundary_),
    ...
    [[nodiscard]] const PublicVisibleNodes& concrete_nodes() const noexcept {
    ...
    [[nodiscard]] NeighbourAddress neighbour_address(Index public_cell) const {
        if (public_cell >= this->size()) {
            throw std::out_of_range(
                "HighVoronoiMesh public neighbour cell index out of range.");
        }
    ...
    internal_neighbour_address(Index internal_cell) const {
        if (internal_cell >= internal_nodes_.size()) {
            throw std::out_of_range(
                "HighVoronoiMesh internal neighbour cell index out of range.");
        }
    ...
    /**
     * Allocation-reusing visible-cell neighbour computation with caller-owned
     * scratch.  The inherited one-argument overload uses this mesh's persistent
     * workspace instead.
     */
    ...
    [[nodiscard]] Address store_internal_vertex(
        ...
        this->mark_neighbours_dirty(internal_sigma);
        return address;
    }
    ...
    [[nodiscard]] bool erase_internal_vertex(
        ...
        const bool erased = database_.erase(address, internal_sigma);
        if (erased) {
            this->mark_neighbours_dirty(internal_sigma);
        }
        return erased;
...
private:
    ...
    [[nodiscard]] Index append_internal_node(
        ...
        const Index appended = internal_nodes_.append_stored(point);
        if (appended != internal) {
            throw std::logic_error(
                "CompositeVoronoiNodes returned a non-contiguous internal index.");
        }
    ...
    void copy_internal_node(Index internal_node, NodePoint& target) const {
        require_internal_node(internal_node);
        internal_nodes_.copy_node(internal_node, target.data());
    }
...
public:
    ...
    [[nodiscard]] Index stable_internal_node(Index public_node) const {
        return this->index_mapping().public_to_internal(public_node);
    }
...
private:
    ...
    [[nodiscard]] Database& database_impl() noexcept override { return owner_.database(); }
    [[nodiscard]] const Database& database_impl() const noexcept override { return owner_.database(); }
...
template <class HighMesh, class ComputationMesh, class SearchTree, class SearchData>
[[nodiscard]] bool highvoronoi_vertex_survives_nearest_test(
    ...
    tree.write_point(data, position);
    ...
    const Index nearest_internal =
        computation_mesh.stable_internal_node(nearest->index);
...
HighVoronoiMesh<NodeScalarT, Dim, DatabaseT>::append_engine(
    ...
    for (Index local = Index{0}; local < engine->node_count(); ++local) {
        ...
        if (!external_boundary().contains(node, boundary_tolerance)) {
            throw std::domain_error(
                "Visible compute-engine node lies outside the external boundary.");
        }
    }

    if (engine->node_count() > invalid_index() - node_offset) {
        throw std::length_error(
            "Compute-engine node range exceeds HighVoronoi Index capacity.");
    }
    const Index new_internal_count =
        static_cast<Index>(node_offset + engine->node_count());
    validate_internal_capacity(new_internal_count);

    const Index retained_offset = internal_nodes_.append_engine(engine);
    if (retained_offset != node_offset) {
        throw std::logic_error(
            "CompositeVoronoiNodes returned an unexpected compute-engine offset.");
    }

    const ShiftMask zero_shift(
        external_boundary().size(),
        std::uint8_t{0});
    for (Index local = Index{0}; local < engine->node_count(); ++local) {
        const Index internal = static_cast<Index>(node_offset + local);
        active_.push_back(true);
        visible_.push_back(true);
        references_.push_back(std::nullopt);
        reference_shifts_.push_back(zero_shift);
        primary_address_lists_.emplace_back();
        secondary_address_lists_.emplace_back();
        (void)this->index_mapping().append_visible(internal);
    }
    neighbour_storage_.resize(static_cast<std::size_t>(new_internal_count));
    report.appended_nodes = static_cast<std::size_t>(engine->node_count());
    ...
    // Register the engine's immutable native neighbour records in the same
    // logical neighbour address space as stored records. Each newly appended
    // engine cell publishes its virtual baseline address immediately, but the
    // dirty bit deliberately remains unchanged: a surrounding HighVoronoi
    // computation may still have to reconcile that engine-local topology with
    // previously existing or subsequently inserted nodes.
    if (engine->provides_neighbours()) {
        const auto neighbour_registration =
            neighbour_storage_.database().register_engine(engine, node_offset);
        for (Index local = Index{0}; local < engine->node_count(); ++local) {
            const Index internal = static_cast<Index>(node_offset + local);
            neighbour_storage_.publish_address(
                internal,
                neighbour_registration.first_address +
                    static_cast<Address>(local));
        }
    }
