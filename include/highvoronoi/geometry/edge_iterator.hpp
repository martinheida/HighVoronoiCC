...
public:
    ...
    [[nodiscard]] const Point& direction() const noexcept {
        return *direction_;
    }

    [[nodiscard]] Scalar cycle_error() const noexcept {
        return cycle_error_;
    }
    ...
    EdgeIterator(
        ...
        : nodes_(&nodes),
          ...
          node_buffer_(detail::make_normal_point<Scalar, DimensionAtCompileTime>(
              dimension_)),
          origin_buffer_(detail::make_normal_point<Scalar, DimensionAtCompileTime>(
              dimension_)),
          span_buffer_(detail::make_normal_point<Scalar, DimensionAtCompileTime>(
              dimension_)),
    ...
    [[nodiscard]] std::vector<Index> valid_generators() const {
        ...
        for (std::size_t position = 0;
            ...
            if (level.valid_nodes[position] != 0) {
                result.push_back(level.sig[position]);
            }
        }
        return result;
    }
...
private:
    ...
    [[nodiscard]] std::optional<View> next_general() {
        ...
        bool ok = qr_solver_.solve(
            *nodes_,
            ...
            direction_);
        cycle_error_ = ok
            ? general_direction_error(omitted, direction_)
        ...
        return make_view(input_signature_[omitted]);
    }

    [[nodiscard]] Scalar general_direction_error(
        std::size_t omitted_position,
        const Point& direction) {

        std::size_t origin_position = input_signature_.size() - 1;
        if (origin_position == omitted_position) {
            --origin_position;
        }

        copy_global_node(input_signature_[origin_position], origin_buffer_);
        ...
        for (std::size_t position = 0;
            ...
            copy_global_node(input_signature_[position], node_buffer_);
            span_buffer_ = node_buffer_ - origin_buffer_;
            const Scalar norm = span_buffer_.norm();
            if (!(norm > Scalar{0})) {
                return (std::numeric_limits<Scalar>::max)();
            }
            error += std::abs(span_buffer_.dot(direction) / norm);
        }
        return error;
    }
    ...
    [[nodiscard]] View make_view(Index skip) const {
        return View{
            EdgeIndexView<Index>(
