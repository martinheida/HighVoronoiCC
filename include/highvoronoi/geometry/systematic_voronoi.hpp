...
public:
    ...
    void compute() {
        ...
        while (current_cell() <= last_local_cell_) {
            ...
            if (cell == last_local_cell_) {
                ...
                edge_hash_.clear();
                current_cell_.store(
                    static_cast<Index>(cell + Index{1}),
     ...
     *
     * The order is intentional: queue/hash/boundary state belongs to the old
     * published cell until the complete reset has finished.
     */
