...
public:
    ...
    VoronoiWorker(
        ...
        : mesh_(mesh),
          ...
          infinite_origin_buffer_(make_mesh_point()),
          infinite_direction_buffer_(make_mesh_point()),
...
private:
    ...
    void systematic_explore_vertex(
        ...
        while (const auto edge = searching_iterator_.next()) {
            ...
            // Julia performs semantic verify_vertex() only for the descent
            // bootstrap vertex.  WalkRay endpoints are already constrained by
            // the raycast itself and are stored through the duplicate-safe
            // mesh/database path.  The stronger historical C++ re-verification
            // remains available as an explicit diagnostic option.
            if (raycaster_.parameters().verify_walk_vertices &&
                !mesh_.contains_vertex(
                    candidate_vertex_.sigma,
                    internal_sigma_buffer_) &&
    ...
    [[nodiscard]] bool walk_ray(
        ...
        if (result.status == RayCastStatus::Infinite) {
            ...
            detail::copy_voronoi_worker_point(
                origin.position,
                infinite_origin_buffer_);
            detail::copy_voronoi_worker_point(
                edge.direction(),
                infinite_direction_buffer_);
    ...
    MeshPoint infinite_origin_buffer_;
    MeshPoint infinite_direction_buffer_;
