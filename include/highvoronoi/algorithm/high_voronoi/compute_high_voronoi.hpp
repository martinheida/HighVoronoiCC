...
#include <highvoronoi/algorithm/high_voronoi/periodic_boundary_repair.hpp>
...
public:
    ...
    /**
     * @brief Execute the complete HighVoronoi transaction.
     *
     * With `verbose=true`, orchestration prints coarse phase/timing messages
     * while every delegated RefineVoronoi/RemoveVoronoi/sparse-repair compute
     * receives the same flag and therefore owns its normal cell-level meter.
     * There is intentionally no synthetic total-percent meter across rounds.
     */
    [[nodiscard]] Report compute(bool verbose = false) {
        Report report;
        const TimePoint total_start = Clock::now();
        std::size_t periodization_round = 0;
        ...
        if (!deleted.empty()) {
            const TimePoint stage_start = begin_stage(
                "Removing pending nodes",
                verbose);
        ...
        if (!visible_new.empty()) {
            const bool initial = mesh_.integrated_node_count() == Index{0};
            const std::string_view label = initial
                ? std::string_view{"Computing basic mesh"}
                : std::string_view{"Refining visible mesh"};
            const TimePoint stage_start = begin_stage(label, verbose);
            requests = refine_existing_block(
                visible_new,
                mesh_.integrated_node_count(),
                report,
                verbose);
            end_stage(label, stage_start, total_start, verbose);
        ...
        if (remover) {
            const TimePoint stage_start = begin_stage(
                "Recomputing after removal",
                verbose);
            (void)remover->compute(verbose);
        ...
        const bool has_periodic_repair =
            !periodic_repair_seed_signatures_.empty();
        TimePoint repair_start{};
        if (has_periodic_repair) {
            repair_start = begin_stage("Periodic boundary repair", verbose);
        }
        ...
        // HighVoronoi compute is the externally visible transaction boundary.
        // Refine/remove substeps may already have propagated intermediate dirty
        // state; this final OR also captures changes from periodic sparse repair
        // and guarantees that no completed compute can be missed by a consumer.
...
private:
    ...
    [[nodiscard]] TimePoint begin_stage(
        std::string_view label,
        bool verbose) const {
        if (verbose) {
            std::cout << "[HighVoronoi] " << label << "\n";
            std::cout.flush();
        }
        return Clock::now();
    }
    ...
    void end_stage(
        std::string_view label,
        const TimePoint& stage_start,
        const TimePoint& total_start,
        bool verbose) const {
        if (!verbose) {
            return;
        }
        std::ostringstream line;
        line << "[HighVoronoi] " << label
             << " finished in "
             << std::fixed << std::setprecision(3)
             << seconds_since(stage_start) << " s"
             << " (total " << seconds_since(total_start) << " s)\n";
        std::cout << line.str();
        std::cout.flush();
    }

    [[nodiscard]] static double seconds_since(
    ...
    void close_periodic_requests(
        ...
        Report& report,
        const TimePoint& total_start,
        std::size_t& periodization_round,
        bool verbose) {
        while (!requests.empty()) {
            ...
            const std::string label =
                "Periodization round #" +
                std::to_string(periodization_round);
            const TimePoint stage_start = begin_stage(label, verbose);
