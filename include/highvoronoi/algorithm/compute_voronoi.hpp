...
#include <highvoronoi/algorithm/systematic_voronoi.hpp>
...
public:
    ...
    using BranchMesh = std::conditional_t<
        ...
        ParallelBranchMesh,
        Mesh>;
    using BranchVertexPoint = typename BranchMesh::VertexPoint;
     ...
     *
     * @param verbose Enable one cell-level ProgressMeter for the complete
     *        selected compute range. The meter is owned here, not by workers or
     *        higher-level refine/periodization orchestration.
     */
    void compute(bool verbose = false) {
        begin_progress(verbose);
    ...
    /**
     * @brief Notify the top-level coordinator that one branch finished one cell.
     *
     * SystematicVoronoi deliberately knows nothing about ProgressMeter. It only
     * reports the cell-completion event to its ComputeVoronoi parent. Several
     * mesh branches may call this concurrently; ProgressMeter::increase() is
     * thread-safe.
     */
    void cell_completed() {
        if (progress_) {
    ...
    [[nodiscard]] bool register_vertex(
        ...
        BranchVertexPoint& mesh_point_buffer) {
...
private:
    void begin_progress(bool verbose) {
        progress_.reset();
        if (!verbose) {
            return;
        }
        ...
        progress_ = std::make_unique<detail::ProgressMeter>(
            total_cells,
    ...
    Index range_end_;
    std::unique_ptr<detail::ProgressMeter> progress_;
