...
public:
     ...
     *
     * `verbose` is forwarded only to the delegated ComputeVoronoi NEW-cell
     * phase; RefineVoronoi does not maintain a second overall progress meter.
     */
    [[nodiscard]] AffectedVector& compute(bool verbose = false) {
        ...
        // Lifecycle boundary: every consumer tracker must retain all cells
        // touched by this completed refine, independently of later neighbour
        // recomputation clearing the mesh-owned dirty bits.
        mesh_.propagate_neighbour_dirty();
        return affected_;
    }
