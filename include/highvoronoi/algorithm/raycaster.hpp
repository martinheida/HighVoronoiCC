 ...
 * - CombinedRayCaster: Julia's Raycast_Combined path. Its nanoflann KD-tree
 *   traversal is allowed to move the candidate endpoint, unwind completely,
 *   and restart from the root with caller-owned traversal state.
 *
...
/**
 * @brief Policy for CombinedRaycast when its conservative far-field
 *        plane-degeneracy guard rejects at least one candidate and the
 *        combined search would otherwise terminate as an infinite edge.
 *
 * FastLossy preserves the original conservative Combined behavior: if a
 * precision-critical candidate has to be rejected and the search would
 * otherwise terminate as infinite, the ray remains infinite. Robust reruns
 * the complete cast with the configured fallback algorithm. A concrete
 * candidate whose ray parameter is NaN or +/-Inf is handled by the same
 * policy: FastLossy returns an infinite edge, Robust reruns the cast. Finite
 * negative intermediate ray parameters retain the original Combined flow.
 */
enum class CombinedPrecisionPolicy {
    FastLossy,
    Robust
};

/**
 * @brief Select the complete fallback algorithm used by CombinedRaycast.
 *
 * InRangeOnly always uses InRangeRayCaster.
 * ClassicGeneralInRangeDegenerate uses ClassicRayCaster for ordinary
 * d-generator WalkRay edges and InRangeRayCaster for degenerate edges
 * with more than d supporting generators (and for non-WalkRay calls).
 */
enum class CombinedFallbackMethod {
    InRangeOnly,
    ClassicGeneralInRangeDegenerate
};

struct CombinedRaycastOptions {
    // Robust is the public default: Combined keeps its fast path, but if the
    // conservative far-field/non-finite candidate handling cannot decide the
    // cast reliably, the complete cast is rerun with the configured fallback.
    // FastLossy remains available only by explicit opt-in.
    CombinedPrecisionPolicy precision_policy =
        CombinedPrecisionPolicy::Robust;
    CombinedFallbackMethod fallback_method =
        CombinedFallbackMethod::ClassicGeneralInRangeDegenerate;

    // Diagnostic only. Disabled by default so normal Combined timings do not
    // pay a per-cast counter update. The benchmark can enable this explicitly.
    bool collect_statistics = false;
};

/**
 * @brief Optional diagnostic counters for CombinedRayCaster.
 *
 * The counters are worker-local and non-atomic. A RayCaster is already
 * worker-local, so collecting them adds no synchronization. They are updated
 * only when CombinedRaycastOptions::collect_statistics is true.
 */
struct CombinedRaycastStatistics {
    std::uint64_t combined_casts = 0;

    // Candidate-level hits of the conservative far-field plane guard.
    std::uint64_t precision_guard_hits = 0;
    // Casts in which that guard rejected at least one candidate.
    std::uint64_t precision_guard_casts = 0;
    // Robust reruns because a guarded cast ultimately ended Infinite.
    std::uint64_t precision_guard_fallbacks = 0;

    // Concrete candidates for which get_t_with_error returned non-finite t.
    std::uint64_t candidate_pos_inf = 0;
    std::uint64_t candidate_neg_inf = 0;
    std::uint64_t candidate_nan = 0;

    // Final Combined point was non-finite despite finite candidate t values.
    std::uint64_t nonfinite_position_results = 0;

    // Complete Robust reruns, split by selected fallback algorithm.
    std::uint64_t total_fallbacks = 0;
    std::uint64_t classic_fallbacks = 0;
    std::uint64_t inrange_fallbacks = 0;
};
...
/**
 * @brief Julia-faithful `Raycast_Combined` implementation for nanoflann.
 *
 * Algorithmic reference chain:
 *
 * - `src/raycast.jl::raycast_des2(..., ::Raycast_Combined, ...)`
 * - `src/searchtrees.jl::reset!(::NNSearchData, ...)`
 * - `src/extended.jl::search_vertex2`
 * - `src/searchtrees.jl::skip_nodes_on_search`
 ...
 *
 * The KD traversal itself is implemented directly inside the vendored
 * nanoflann `KDTreeSingleIndexAdaptor`.  This class owns the worker-local
 * counterpart of Julia `NNSearchData` and passes it to that traversal by
 * reference.  The only intentional numerical substitution is Julia's
 * `Double64`/`HPCorrector`: at the two corresponding correction sites C++
 * reuses `RayCasterBase::correct_cast`, i.e. the existing CPQR corrector with
 * software-Float128 fallback.  The search/restart algorithm is unchanged.
 */
template <class SearchTreeT>
class CombinedRayCaster final
    : public RayCasterBase<
          CombinedRayCaster<SearchTreeT>,
          SearchTreeT,
          RaycastParameters<CombinedRaycast, typename SearchTreeT::Scalar>> {
public:
    using Parameters =
        RaycastParameters<CombinedRaycast, typename SearchTreeT::Scalar>;
    using Base = RayCasterBase<
    ...
    using ClassicFallback = ClassicRayCaster<SearchTreeT>;
    using ClassicParameters = typename ClassicFallback::Parameters;
    using InRangeFallback = InRangeRayCaster<SearchTreeT>;
    using InRangeParameters = typename InRangeFallback::Parameters;

    explicit CombinedRayCaster(
        const SearchTreeT& tree,
        Parameters parameters = Parameters{},
        CombinedRaycastOptions options = CombinedRaycastOptions{})
        : Base(tree, parameters),
          options_(options),
          combined_data_(
              static_cast<std::size_t>(this->tree_.dimension()),
              this->tree_.combined_tree_node_count()) {
        const std::size_t reserve_count = std::max<std::size_t>(
            std::size_t{8}, std::size_t{2} * this->dimension_size() + 4);
        combined_data_.sigma.reserve(reserve_count);
        combined_data_.taboo.reserve(this->dimension_size() + 1);
        combined_data_.ts.reserve(reserve_count);
        combined_data_.cs.reserve(reserve_count);
        combined_data_.full_errors.reserve(reserve_count);

        // FastLossy never performs a fallback cast. Keep its object/state as
        // close as possible to the original Combined implementation; only the
        // Robust policy owns prepared fallback raycasters. Their construction
        // happens during algorithm setup, not when a rare fallback is needed.
        if (options_.precision_policy == CombinedPrecisionPolicy::Robust) {
            classic_fallback_ = std::make_unique<ClassicFallback>(
                tree, make_fallback_parameters<ClassicParameters>(parameters));
            inrange_fallback_ = std::make_unique<InRangeFallback>(
                tree, make_fallback_parameters<InRangeParameters>(parameters));
        }
    }
    ...
    void activate_cell(
        Index cell,
        const NeighbourContainer& neighbours) {
        Base::activate_cell(cell, neighbours);

        // Keep the normal Combined hot path free of fallback activation cost.
        // The fallback raycasters are synchronized lazily only if this cell
        // actually needs a complete fallback cast.
        if (options_.precision_policy == CombinedPrecisionPolicy::Robust) {
            active_cell_ = cell;
            fallback_neighbours_.assign(neighbours.begin(), neighbours.end());
            classic_fallback_active_ = false;
            inrange_fallback_active_ = false;
        }
    }

    [[nodiscard]] CombinedRayCaster safe_copy_impl() const {
        return CombinedRayCaster(
            this->tree_, this->parameters_, options_);
    }

    template <class NewMesh>
    [[nodiscard]] auto rebind(NewMesh& mesh) const {
        auto tree = this->tree_.rebind(mesh);
        return CombinedRayCaster<decltype(tree)>(
            tree, this->parameters_, options_);
    }

    [[nodiscard]] const CombinedRaycastOptions& combined_options() const noexcept {
        return options_;
    }

    [[nodiscard]] const CombinedRaycastStatistics&
    combined_statistics() const noexcept {
        return statistics_;
    }

    void reset_combined_statistics() noexcept {
        statistics_ = CombinedRaycastStatistics{};
    }
    ...
    [[nodiscard]] Result cast_impl(
        ...
        if (options_.collect_statistics) {
            ++statistics_.combined_casts;
        }

        // Julia raycast_des2(::Raycast_Combined): project the old point twice
        // onto the ray through the edge reference generator.  The second
        // projection is retained deliberately although it is normally a no-op.
        this->load_node(edge[0], this->reference_node_);
        const Point& x0 = this->reference_node_;
        this->delta_ = x0 - start;
        this->query_point_ =
            start + direction * direction.dot(this->delta_);
        this->delta_ = x0 - this->query_point_;
        this->query_point_ +=
            direction * direction.dot(this->delta_);
        ...
        try {
            search_vertex2(edge, usage);
        } catch (const NonfiniteCandidateRequest&) {
            if (options_.precision_policy == CombinedPrecisionPolicy::Robust) {
                return fallback_cast(
                    sigma, start, direction, edge, origin, usage, direction_error);
            }
            return infinite_result(start);
        }

        const Index generator = combined_data_.bestnode;
        const bool combined_would_be_infinite =
            generator == invalid ||
            std::binary_search(origin.begin(), origin.end(), generator);
        if (combined_would_be_infinite) {
            if (combined_data_.precision_guard_rejected &&
                options_.precision_policy == CombinedPrecisionPolicy::Robust) {
                if (options_.collect_statistics) {
                    ++statistics_.precision_guard_fallbacks;
                }
                return fallback_cast(
                    sigma, start, direction, edge, origin, usage, direction_error);
            }
        ...
        this->query_point_ = combined_data_.new_r;
        if (!this->query_point_.allFinite()) {
            if (options_.collect_statistics) {
                ++statistics_.nonfinite_position_results;
            }
            if (options_.precision_policy == CombinedPrecisionPolicy::Robust) {
                return fallback_cast(
                    sigma, start, direction, edge, origin, usage, direction_error);
        ...
        sigma.insert(
            sigma.end(),
            combined_data_.sigma.begin(),
            combined_data_.sigma.end());
        detail::sort_unique(sigma);

        // User-confirmed C++ interpretation of Julia's now-obsolete
        // variance_tol(type) experiment: retain the established C++ threshold.
        const bool variance_requests_correction =
            this->parameters_.variance_tolerance * Scalar{1e-5} <
            this->vertex_variance(sigma, this->query_point_);

        bool corrected = combined_data_.corrected;
        if (variance_requests_correction &&
            this->correct_cast(
                this->query_point_, edge, generator,
                usage, this->corrected_point_)) {
            this->query_point_ = this->corrected_point_;
        ...
        // Julia Raycast_Combined returns t=1.0 for every finite result; it is a
        // success marker rather than the geometric ray parameter in this path.
        return Result{
            RayCastStatus::Finite,
            generator,
            Scalar{1},
...
private:
    struct NonfiniteCandidateRequest {};

    template <class TargetParameters>
    [[nodiscard]] static TargetParameters make_fallback_parameters(
        const Parameters& source) {
        TargetParameters target{};
        target.variance_tolerance = source.variance_tolerance;
        target.break_tolerance = source.break_tolerance;
        target.boundary_node_tolerance = source.boundary_node_tolerance;
        target.plane_tolerance = source.plane_tolerance;
        target.ray_tolerance = source.ray_tolerance;
        target.rank_tolerance = source.rank_tolerance;
        target.verification_absolute_tolerance =
            source.verification_absolute_tolerance;
        target.verification_relative_tolerance =
            source.verification_relative_tolerance;
        target.verify_walk_vertices = source.verify_walk_vertices;
        target.vertex_condition_tolerance =
            source.vertex_condition_tolerance;
        target.vertex_correction_relative_tolerance =
            source.vertex_correction_relative_tolerance;
        target.vertex_correction_max_iterations =
            source.vertex_correction_max_iterations;
        target.classic_relative_error_trigger =
            source.classic_relative_error_trigger;
        target.classic_absolute_error_trigger =
            source.classic_absolute_error_trigger;
        target.inrange_t_slack = source.inrange_t_slack;
        return target;
    }

    template <class Sigma, class Edge, class Origin>
    [[nodiscard]] Result fallback_cast(
        Sigma& sigma,
        const Point& start,
        const Point& direction,
        const Edge& edge,
        ...
        RayCastUsage usage,
        Scalar direction_error) {
        const bool use_classic =
            options_.fallback_method ==
                CombinedFallbackMethod::ClassicGeneralInRangeDegenerate &&
            usage == RayCastUsage::WalkRay &&
            static_cast<std::size_t>(sigma.size()) == this->dimension_size();

        if (options_.collect_statistics) {
            ++statistics_.total_fallbacks;
            if (use_classic) {
                ++statistics_.classic_fallbacks;
            } else {
                ++statistics_.inrange_fallbacks;
            }
        }

        if (use_classic) {
            if (!classic_fallback_active_) {
                classic_fallback_->activate_cell(
                    active_cell_, fallback_neighbours_);
                classic_fallback_active_ = true;
            }
            return classic_fallback_->cast(
                sigma, start, direction, edge, origin, usage, direction_error);
        }

        if (!inrange_fallback_active_) {
            inrange_fallback_->activate_cell(
                active_cell_, fallback_neighbours_);
            inrange_fallback_active_ = true;
        }
        return inrange_fallback_->cast(
            sigma, start, direction, edge, origin, usage, direction_error);
    }

    struct CombinedSearchData {
        Point r;
        Point r0;
        Point x0;
        Point new_r;
        ...
        std::vector<Index> taboo;
        std::vector<std::uint8_t> visited_leafs;
        std::vector<Scalar> ts;
        std::vector<Scalar> cs;
        std::vector<Scalar> full_errors;

        Scalar t = Scalar{0};
        Scalar bestdist = infinity_value();
        Index bestnode = invalid_index_value();
        Scalar c = Scalar{0};
        Scalar horizontal_error = Scalar{0};
        Scalar current_c = Scalar{0};
        std::size_t visited = 0;
        Scalar dist_r_x0_2 = Scalar{0};
        Scalar dist_new_r_x0_2 = infinity_value();
        Scalar plane_tolerance = Scalar{0};
        Scalar du = Scalar{0};
        Scalar horizontal = Scalar{0};
        Scalar full_error_t = Scalar{0};
        Scalar upper_t = infinity_value();
        bool full_mode = false;
        bool full_edge = false;
        bool corrected = false;
        bool precision_guard_rejected = false;

        CombinedSearchData(
            std::size_t dimension,
            std::size_t tree_nodes)
            : r(make_point(dimension)),
              r0(make_point(dimension)),
              x0(make_point(dimension)),
              new_r(make_point(dimension)),
              u(make_point(dimension)),
              mins(make_point(dimension)),
              maxs(make_point(dimension)),
              box_mins(make_point(dimension)),
              box_maxs(make_point(dimension)),
              visited_leafs(tree_nodes, std::uint8_t{0}) {}

        [[nodiscard]] static Point make_point(std::size_t dimension) {
            if constexpr (Base::DimensionAtCompileTime == Dynamic) {
        ...
        [[nodiscard]] static constexpr Scalar infinity_value() noexcept {
            if constexpr (std::numeric_limits<Scalar>::has_infinity) {
                return std::numeric_limits<Scalar>::infinity();
            } else {
                return (std::numeric_limits<Scalar>::max)();
        ...
        [[nodiscard]] static constexpr Index invalid_index_value() noexcept {
            return (std::numeric_limits<Index>::max)();
        }
    };

    /** C++ counterpart of `reset!(::NNSearchData, ...)` in searchtrees.jl. */
    template <class Origin, class Edge, class Sigma>
    void reset_combined_data(
        const Origin& origin,
        const Point& r,
        const Point& x0,
        const Point& direction,
        const Edge& /* edge */,
        const Sigma& full_edge,
        RayCastUsage usage,
        Scalar direction_error) {
        auto& data = combined_data_;
        data.sigma.clear();
        data.ts.clear();
        data.cs.clear();
        data.full_errors.clear();
        data.du = direction_error;
        data.t = Scalar{0};
        data.bestdist = Base::infinity();
        data.bestnode = invalid_index();
        data.taboo.assign(origin.begin(), origin.end());
        data.u = direction;
        data.r0 = r;
        data.r = r;
        data.new_r = r;
        data.x0 = x0;
        data.visited = 0;
        data.plane_tolerance = this->parameters_.plane_tolerance;

        const Scalar c1 = this->maximum_projection(origin, direction);
        data.c = c1 + std::abs(c1) * data.plane_tolerance;
        data.current_c = x0.dot(direction);
        data.dist_r_x0_2 = data.horizontal = (r - x0).squaredNorm();
        data.dist_new_r_x0_2 = Base::infinity();
        data.full_error_t = Scalar{0};
        std::fill(
            data.visited_leafs.begin(),
            data.visited_leafs.end(),
            std::uint8_t{0});

        Scalar min_measure = (x0 - r).norm();
        Scalar max_measure = min_measure;
        for (const Index index : full_edge) {
            this->load_node(index, this->candidate_node_);
            const Scalar measure = (this->candidate_node_ - r).norm();
            min_measure = std::min(min_measure, measure);
            max_measure = std::max(max_measure, measure);
        }
        data.horizontal_error = std::abs(max_measure - min_measure);
        data.upper_t = Base::infinity();
        data.full_mode = false;
        data.full_edge = usage == RayCastUsage::WalkRay;
        data.corrected = false;
        data.precision_guard_rejected = false;
        data.box_mins = this->bounds_min_;
        data.box_maxs = this->bounds_max_;
    }

    /**
     * C++ counterpart of `search_vertex2` in `src/extended.jl`.
     * Boundary mirrors are evaluated first with the same candidate routine;
     * only afterwards is Julia's flexible ordinary-node KD search entered.
     */
    template <class Edge>
    void search_vertex2(const Edge& edge, RayCastUsage usage) {
        auto evaluator = [this, &edge, usage](
            Index index,
            Scalar squared_distance) {
            skip_nodes_on_search(
                index, squared_distance, edge, usage);
        };

        auto& data = combined_data_;
        const Point old_r = data.new_r;
        const auto& nodes = this->tree_.extended_nodes();
        const Index mirror_count = nodes.active_boundary_size();
        for (Index position = Index{0}; position < mirror_count; ++position) {
            const Index index = nodes.active_boundary_index(position);
            this->load_node(index, this->candidate_node_);
            const Scalar squared_distance =
                (data.new_r - this->candidate_node_).squaredNorm();
            evaluator(index, squared_distance);
        }
        if (!points_equal(old_r, data.new_r)) {
            data.r = data.new_r;
        }

        this->tree_.combined_search(data, evaluator);
    }

    /**
     * C++ counterpart of `skip_nodes_on_search` in `src/searchtrees.jl`.
     *
     * Julia's `vertex_calculation_hp(..., Double64)` is intentionally replaced
     * only at its correction site by the existing C++ `correct_cast`: same
     * edge+generator support, CPQR in ordinary precision, direct Float128
     * fallback when conditioning/convergence requires it.
     */
    template <class Edge>
    void skip_nodes_on_search(
        Index index,
        Scalar new_dist,
        const Edge& edge,
        RayCastUsage usage) {
        ...
        if (data.visited < data.taboo.size() &&
            std::binary_search(
                data.taboo.begin(), data.taboo.end(), index)) {
            ++data.visited;
            return;
        }

        this->load_node(index, this->candidate_node_);
        const Point& x_new = this->candidate_node_;

        const Scalar c_new = x_new.dot(data.u);
        if (c_new <= data.c) {
            return;
        }

        auto [new_t, full_error] = this->get_t_with_error(
            data.r0, data.u, data.x0, x_new, data.du);

        // A non-finite parameter for a concrete forward candidate is distinct
        // from the normal Combined end state bestnode==invalid: here a
        // candidate exists, but its intersection cannot be represented
        // reliably. FastLossy conservatively terminates this ray as Infinite;
        // Robust reruns the complete cast with the configured fallback.
        //
        // Do NOT special-case finite new_t < 0 here. Combined measures new_t
        // from its projected internal reference point r0, not directly from
        // the original ray start, and the original algorithm permits such
        // intermediate values to continue through the normal state machine.
        if (!std::isfinite(new_t)) {
            if (options_.collect_statistics) {
                if (std::isnan(new_t)) {
                    ++statistics_.candidate_nan;
                } else if (new_t > Scalar{0}) {
                    ++statistics_.candidate_pos_inf;
                } else {
                    ++statistics_.candidate_neg_inf;
                }
            }
            throw NonfiniteCandidateRequest{};
        }

        if (new_t > data.upper_t) {
            return;
        }

        const Scalar relative_plane_tolerance =
            std::abs(full_error / new_t);
        const Scalar this_error = full_error + data.horizontal_error;
        const Scalar old_distance = data.dist_new_r_x0_2;
        const Scalar correction = std::max(
            old_distance * Scalar{10} * relative_plane_tolerance,
            Scalar{10} * this_error);
        const Scalar this_correction =
            std::max(correction, data.full_error_t);

        if (new_dist > old_distance + this_correction) {
            return;
        }

        this->delta_ = x_new - data.x0;
        const Scalar abs_delta_x = this->delta_.squaredNorm();
        const Scalar scale = point_in_box(
            data.r0, data.box_mins, data.box_maxs)
            ? Scalar{1}
            : std::sqrt(data.dist_r_x0_2 / (new_t * new_t));

        if (std::abs(new_dist - old_distance) <
            std::max(correction, data.full_error_t)) {
            if (abs_delta_x / new_dist < Scalar{100} * correction) {
                return;
            }
            data.sigma.push_back(index);
            data.cs.push_back(c_new);
            data.ts.push_back(new_t);
            data.full_errors.push_back(full_error);
            data.full_error_t = std::max(
                std::min(Scalar{1}, Scalar{10} * scale) *
                    std::abs(new_dist - old_distance),
                data.full_error_t);
        ...
        const bool full_mode =
            relative_plane_tolerance > Scalar{1e-10} ||
            full_error > Scalar{1e-8} /
                std::max(scale, Scalar{1e-4});

        if (abs_delta_x / (new_t * new_t) < relative_plane_tolerance) {
            // Keep the original Combined behavior locally: reject only this
            // candidate and continue searching. Robust mode merely remembers
            // that a plausible far-field candidate was suppressed. Only if
            // the complete Combined cast later terminates as Infinite is the
            // cast rerun with the configured fallback.
            if (options_.collect_statistics) {
                ++statistics_.precision_guard_hits;
                if (!data.precision_guard_rejected) {
                    ++statistics_.precision_guard_casts;
                }
            }
            if (options_.precision_policy == CombinedPrecisionPolicy::Robust ||
                options_.collect_statistics) {
                data.precision_guard_rejected = true;
            }
            return;
        }

        Point new_r = data.r0 + new_t * data.u;

        // Julia vertex_calculation_hp(data, new_r, u, x_new, full_mode).
        // The user explicitly requested reuse of the established C++ vertex
        // corrector here instead of reproducing the old Double64 implementation.
        if (full_mode &&
            this->correct_cast(
                new_r, edge, index, usage, this->corrected_point_)) {
            new_r = this->corrected_point_;
            data.corrected = true;
        }

        data.t = (new_r - data.r0).dot(data.u);
        data.new_r = new_r;
        const Scalar distance_new_r_x0 = (new_r - data.x0).norm();
        const Scalar distance_new_r_x0_2 =
            distance_new_r_x0 * distance_new_r_x0;
        data.dist_new_r_x0_2 = distance_new_r_x0_2;
        data.full_error_t =
            std::min(Scalar{1}, Scalar{10} * scale) *
            std::max(
                std::abs(
                    distance_new_r_x0_2 -
                    (x_new - new_r).squaredNorm()),
                this_error);
        data.bestdist =
            distance_new_r_x0_2 +
            std::min(Scalar{1}, Scalar{10} * scale) * data.full_error_t;
        data.bestnode = index;
        data.current_c = c_new;
    ...
    [[nodiscard]] static bool point_in_box(
        const Point& point,
        const Point& mins,
        const Point& maxs) noexcept {
    ...
    [[nodiscard]] static bool points_equal(
        const Point& left,
    ...
    std::unique_ptr<ClassicFallback> classic_fallback_;
    std::unique_ptr<InRangeFallback> inrange_fallback_;
    CombinedRaycastOptions options_;
    CombinedRaycastStatistics statistics_{};
    CombinedSearchData combined_data_;

    Index active_cell_ = invalid_index();
    std::vector<Index> fallback_neighbours_;
    bool classic_fallback_active_ = false;
    bool inrange_fallback_active_ = false;
};
...
template <class SearchTree>
[[nodiscard]] CombinedRayCaster<SearchTree> make_raycaster(
    const SearchTree& tree,
    RaycastParameters<CombinedRaycast, typename SearchTree::Scalar> parameters) {
    return CombinedRayCaster<SearchTree>(
        tree, std::move(parameters), CombinedRaycastOptions{});
}
...
[[nodiscard]] CombinedRayCaster<SearchTree> make_raycaster(
    const SearchTree& tree,
    RaycastParameters<CombinedRaycast, typename SearchTree::Scalar> parameters,
    CombinedRaycastOptions options) {
    return CombinedRayCaster<SearchTree>(
        tree, std::move(parameters), options);
}
