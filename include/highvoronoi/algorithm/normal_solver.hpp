...
public:
    ...
    /**
     * @brief Julia-compatible delta_u of the most recent edge-normal solve.
     *
     * The d x d input matrix is the C++ counterpart of Julia's `onb` scratch.
     * Reuse it after QR instead of reloading all generators in EdgeIterator.
     */
    [[nodiscard]] Scalar last_direction_error() const noexcept {
        return static_cast<Scalar>(last_direction_error_);
    }
...
private:
    ...
    [[nodiscard]] bool factor_and_extract(Point& direction) {
        ...
        // Julia General_EdgeIterator.delta_u reuses the `onb` matrix filled by
        // u_qr. `matrix_` is unchanged by qr_.compute(matrix_) and contains
        // exactly those translated span columns.
        WorkScalar error = WorkScalar{0};
        for (std::size_t column = 0;
            ...
            WorkScalar span_norm_squared = WorkScalar{0};
            WorkScalar projection = WorkScalar{0};
            for (std::size_t row = 0;
                 row < dimension_;
                 ++row) {
                const Eigen::Index rr =
                    static_cast<Eigen::Index>(row);
                const WorkScalar span = matrix_(rr, cc);
                span_norm_squared += span * span;
                projection +=
                    span * static_cast<WorkScalar>(
                        direction[rr]);
            }

            using std::sqrt;
            const WorkScalar span_norm = sqrt(span_norm_squared);
            if (!(span_norm > WorkScalar{0}) ||
                !finite(span_norm)) {
                direction.setZero();
                last_direction_error_ =
                    (std::numeric_limits<WorkScalar>::max)();
                return false;
            }

            error += abs_value(projection / span_norm);
        }

        last_direction_error_ = error;
        return finite(last_direction_error_);
    }
    ...
    WorkScalar last_direction_error_{WorkScalar{0}};
...
public:
    ...
    [[nodiscard]] Scalar last_direction_error() const noexcept {
        return core_.last_direction_error();
    }
